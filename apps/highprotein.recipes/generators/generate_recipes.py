...
<body class="min-h-screen bg-slate-50 text-slate-900 font-sans">
    ...
    <main class="pt-32">
        ...
        <section class="py-12 bg-slate-50">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="grid lg:grid-cols-3 gap-8" x-data='recipeSubstitution({alpine_config})'>
                    ...
                    <div class="lg:col-span-1 space-y-6">
                        ...
                        <div class="bg-white rounded-2xl p-6 shadow-md sticky top-40">
                            <div class="flex justify-between items-center mb-6">
                                <h2 class="anton-text text-2xl text-slate-900">INGREDIENTS</h2>
                                <button x-show="checkHasSubstitutions()" x-cloak @click="resetAll()" class="text-xs text-brand-600 hover:text-brand-700 font-semibold uppercase tracking-wider flex items-center gap-1">
                            ...
                            <div x-show="checkHasSubstitutions()" x-cloak class="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-xl text-sm">
                                <div class="flex items-start gap-2">
                                    ...
                                    <div>
                                        <p class="font-semibold text-amber-800"><span x-text="getSubstitutionCount()"></span> ingredient(s) swapped</p>
                                        <template x-if="checkHasHydrationAdjustment()">
                                            <p class="text-amber-700 mt-1">Hydration adjustment: <span x-text="getTotalHydrationAdjustment() > 0 ? '+' + getTotalHydrationAdjustment() : getTotalHydrationAdjustment()"></span>ml liquid</p>
                                        </template>
                                    </div>
                                </div>
                            </div>
                            ...
                            <ul class="space-y-2">
                                <template x-for="ing in currentIngredients" :key="ing.originalId || ing.name">
                                    <li class="relative">
                                        <div :class="ing.id && canSubstitute(ing.originalId) ? 'cursor-pointer hover:bg-slate-50 rounded-lg p-2 -mx-2 transition-colors' : 'p-2 -mx-2'" @click="ing.id && canSubstitute(ing.originalId) && toggleIngredient(ing.id)">
                                            <div class="flex justify-between items-start gap-2">
                                                <div class="flex items-start gap-2 flex-1">
                                                    <span class="w-2 h-2 bg-brand-500 rounded-full mt-2 flex-shrink-0" :class="{{'bg-brand-600': ing.isSwapped}}"></span>
                                                    <span x-text="ing.isSwapped ? getDisplayName(ing.id) : ing.displayAmount || ing.name" :class="ing.isSwapped ? 'text-brand-700 font-semibold' : 'text-slate-700'"></span>
                                                </div>
                                                <template x-if="ing.id && canSubstitute(ing.originalId) && !ing.isSwapped">
                                                    <svg class="w-4 h-4 text-brand-500 flex-shrink-0 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"></path></svg>
                                                </template>
                                                <template x-if="ing.isSwapped">
                                                    <button @click.stop="revertIngredient(ing.originalId)" class="text-slate-400 hover:text-slate-600" title="Revert">
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                                                    </button>
                                                </template>
                                            </div>
                                            <template x-if="ing.isSwapped">
                                                <p class="text-xs text-slate-500 mt-1 ml-4">Originally: <span x-text="ing.displayAmount || getDisplayName(ing.originalId)"></span></p>
                                            </template>
                                        </div>
                                        <!-- Substitutes Dropdown -->
                                        <div x-show="ing.id && isExpanded(ing.id) && canSubstitute(ing.originalId)" x-cloak x-transition class="mt-2 bg-slate-50 rounded-xl p-3 border border-slate-200">
                                            ...
                                            <div class="space-y-1">
                                                <template x-for="sub in getSubstitutes(ing.originalId)" :key="sub.id">
                                                    <button @click.stop="selectSubstitute(ing.originalId, sub.id)" class="w-full text-left px-3 py-2 rounded-lg hover:bg-white transition-colors flex justify-between items-center group" :class="sub.isSpecialSwap ? 'border border-amber-200 bg-amber-50' : ''">
                                                        <span x-text="sub.name" class="text-slate-700 group-hover:text-brand-600"></span>
                                ...
                                <div class="mt-4 pt-4 border-t border-slate-200">
                                    ...
                                    <div class="flex flex-wrap gap-2">
                                        <template x-for="delta in nutritionDeltas" :key="delta.name">
                                            <span class="text-xs px-2 py-1 rounded-full" :class="delta.delta > 0 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'" x-text="delta.name + ': ' + delta.formatted"></span>
                                        </template>
                                    </div>
                                </div>
                            </template>
                        ...
                        <div id="nutrition" class="bg-white rounded-2xl border-4 border-slate-900 p-4 transition-all duration-300" :class="{{'border-brand-500 shadow-lg shadow-brand-100': checkHasSubstitutions()}}">
                            ...
                            <div class="border-b border-slate-900 pb-1 mb-1">
                                <p class="text-sm"><span class="font-bold">{yield_short}</span> servings per recipe</p>
                                <div class="flex justify-between items-baseline">
                                    <p class="text-sm font-bold">Serving size</p>
                                    <p class="text-sm font-bold">{servingSize}</p>
                                </div>
                            </div>
                            <div class="border-b-8 border-slate-900 py-2">
                                <p class="text-sm font-bold">Amount per serving</p>
                                <div class="flex justify-between items-baseline">
                                    <p class="text-4xl font-black">Calories</p>
                                    <p class="text-4xl font-black transition-colors duration-300" :class="{{'text-brand-600': getNutritionCalories() !== baseNutrition.calories}}" x-text="getNutritionCalories()">{calories}</p>
                                </div>
                            </div>
                            ...
                            <div class="border-b border-slate-300 py-1 flex justify-between">
                                <p><span class="font-bold">Total Fat</span> <span x-text="getNutritionFat()" :class="{{'text-brand-600 font-semibold': getNutritionFat() !== baseNutrition.fat}}">{fat}</span>g</p>
                                <p class="font-bold" :class="{{'text-brand-600': getNutritionFat() !== baseNutrition.fat}}" x-text="getDVFat() + '%'"></p>
                            </div>
                            <div class="border-b border-slate-300 py-1 flex justify-between">
                                <p><span class="font-bold">Total Carbohydrate</span> <span x-text="getNutritionCarbs()" :class="{{'text-brand-600 font-semibold': getNutritionCarbs() !== baseNutrition.carbs}}">{carbs}</span>g</p>
                                <p class="font-bold" :class="{{'text-brand-600': getNutritionCarbs() !== baseNutrition.carbs}}" x-text="getDVCarbs() + '%'"></p>
                            </div>
                            <div class="border-b border-slate-300 py-1 pl-4 flex justify-between">
                                <p>Dietary Fiber <span x-text="getNutritionFiber()" :class="{{'text-brand-600 font-semibold': getNutritionFiber() !== baseNutrition.fiber}}">{fiber}</span>g</p>
                                <p class="font-bold" :class="{{'text-brand-600': getNutritionFiber() !== baseNutrition.fiber}}" x-text="getDVFiber() + '%'"></p>
                            </div>
                            <div class="border-b border-slate-300 py-1 pl-4">
                                <p>Total Sugars <span x-text="getNutritionSugar()">{sugar}</span>g</p>
                            </div>
                            <div class="border-b-8 border-slate-900 py-1 flex justify-between">
                                <p><span class="font-bold">Protein</span> <span x-text="getNutritionProtein()" :class="{{'text-brand-600 font-semibold': getNutritionProtein() !== baseNutrition.protein}}">{protein}</span>g</p>
                                <p class="font-bold" :class="{{'text-brand-600': getNutritionProtein() !== baseNutrition.protein}}" x-text="getDVProtein() + '%'"></p>
                            </div>
                            <p class="text-xs mt-2 text-slate-500">*The % Daily Value (DV) tells you how much a nutrient in a serving contributes to a daily diet. 2,000 calories a day is used.</p>
                        ...
                        <div class="bg-white rounded-2xl p-6 shadow-md">
                            <div class="flex items-center justify-between mb-4">
                                <h3 class="font-bold text-slate-900 flex items-center gap-2">
                                    <span class="text-xl font-black bg-gradient-to-r from-red-500 via-yellow-500 to-emerald-500 bg-clip-text text-transparent">P:E</span>
                                    <span class="text-sm text-slate-500 font-normal">Ratio</span>
                                </h3>
                            ...
                            <!-- Gauge Container -->
                            <div class="relative mx-auto" style="width: 200px; height: 120px;">
                                <svg viewBox="0 0 200 120" class="w-full h-full">
                                    <defs>
                                        <!-- Gradient for the gauge arc -->
                                        <linearGradient id="gaugeGradient-{slug}" x1="0%" y1="0%" x2="100%" y2="0%">
                                        ...
                                        <!-- Shadow filter -->
                                        <filter id="gaugeShadow-{slug}" x="-20%" y="-20%" width="140%" height="140%">
                                    ...
                                    <!-- Colored gauge arc -->
                                    <path d="M 20 100 A 80 80 0 0 1 180 100" fill="none" stroke="url(#gaugeGradient-{slug})" stroke-width="16" stroke-linecap="round" filter="url(#gaugeShadow-{slug})"/>

                                    <!-- Tick marks -->
                                    <g stroke="#94a3b8" stroke-width="2">
                                        <line x1="20" y1="100" x2="28" y2="100"/>
                                        <line x1="34" y1="60" x2="41" y2="65"/>
                                        <line x1="68" y1="32" x2="72" y2="40"/>
                                        <line x1="100" y1="20" x2="100" y2="28"/>
                                        <line x1="132" y1="32" x2="128" y2="40"/>
                                        <line x1="166" y1="60" x2="159" y2="65"/>
                                        <line x1="180" y1="100" x2="172" y2="100"/>
                                    </g>

                                    <!-- Tick labels -->
                                    <text x="15" y="115" font-size="9" fill="#94a3b8" text-anchor="middle">0</text>
                                    <text x="34" y="50" font-size="9" fill="#94a3b8" text-anchor="middle">5</text>
                                    <text x="100" y="12" font-size="9" fill="#94a3b8" text-anchor="middle">10</text>
                                    <text x="166" y="50" font-size="9" fill="#94a3b8" text-anchor="middle">15</text>
                                    <text x="185" y="115" font-size="9" fill="#94a3b8" text-anchor="middle">20+</text>

                                    <!-- Zone labels (subtle) -->
                                    <text x="45" y="78" font-size="7" fill="#ef4444" font-weight="600" opacity="0.7">LOW</text>
                                    <text x="88" y="55" font-size="7" fill="#eab308" font-weight="600" opacity="0.7">GOOD</text>
                                    <text x="135" y="55" font-size="7" fill="#22c55e" font-weight="600" opacity="0.7">GREAT</text>

                                    <!-- Needle -->
                                    <g :style="'transform-origin: 100px 100px; transform: rotate(' + (Math.min(20, Math.max(0, getPERatio())) / 20 * 180 - 90) + 'deg); transition: transform 0.8s cubic-bezier(0.34, 1.56, 0.64, 1);'">
                                        <polygon points="100,30 96,100 104,100" fill="#1e293b"/>
                            ...
                            <div class="text-center mt-2">
                                <div class="inline-flex items-baseline gap-1">
                                    <span class="text-4xl font-black transition-colors duration-300" :class="{pe_class_binding}" x-text="getPERatio().toFixed(1)"></span>
                                    <span class="text-lg text-slate-400">: 1</span>
                                </div>
                                <div class="mt-1">
                                    <span class="inline-block px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider transition-colors duration-300"
                                          :class="{{
                                            'bg-emerald-100 text-emerald-700': getPERatio() >= 15,
                                            'bg-green-100 text-green-700': getPERatio() >= 10 && getPERatio() < 15,
                                            'bg-yellow-100 text-yellow-700': getPERatio() >= 5 && getPERatio() < 10,
                                            'bg-orange-100 text-orange-700': getPERatio() < 5
                                          }}"
                                          x-text="getPERating()"></span>
                                </div>
                            </div>
                            ...
                            <div class="mt-4 pt-4 border-t border-slate-100">
                                <p class="text-xs text-slate-500 text-center">
                                    <span class="font-semibold">P:E Ratio</span> = grams of protein per 100 non-protein calories.
                                    <span class="text-emerald-600 font-medium">Higher ratios</span> support fat loss &amp; muscle retention.
                                </p>
                            </div>
                        </div>
                    </div>
...
def generate_recipe_page(recipe):
    ...
    # Build Alpine.js config for ingredient switcher
    structured_ingredients = recipe.get("ingredientsStructured", [])
    alpine_ingredients = []
    for ing in structured_ingredients:
        alpine_ing = {
            "id": ing.get("id"),
            "originalId": ing.get("id"),
            "name": ing.get("name"),
            "amount": ing.get("amount"),
            "unit": ing.get("unit"),
            "displayAmount": ing.get("displayAmount")
        }
        alpine_ingredients.append(alpine_ing)

    # Extract yield number
    yield_num = int(recipe["yield"].split()[0])

    alpine_config = {
        "recipeId": recipe["slug"],
        "yield": yield_num,
        "servingSize": recipe.get("servingSize", "1 serving"),
        "ingredients": alpine_ingredients,
        "baseNutrition": {
    ...
    alpine_config_json = json.dumps(alpine_config)
    ...
    # P:E class binding for dynamic coloring (Alpine.js expression)
    pe_class_binding = "'text-emerald-600': (getNutritionProtein() / ((getNutritionCalories() - getNutritionProtein() * 4) / 100)) >= 15, 'text-green-600': (getNutritionProtein() / ((getNutritionCalories() - getNutritionProtein() * 4) / 100)) >= 10 && (getNutritionProtein() / ((getNutritionCalories() - getNutritionProtein() * 4) / 100)) < 15, 'text-yellow-600': (getNutritionProtein() / ((getNutritionCalories() - getNutritionProtein() * 4) / 100)) >= 5 && (getNutritionProtein() / ((getNutritionCalories() - getNutritionProtein() * 4) / 100)) < 10, 'text-orange-600': (getNutritionProtein() / ((getNutritionCalories() - getNutritionProtein() * 4) / 100)) < 5"
    ...
    html = RECIPE_TEMPLATE.format(
        ...
        alpine_config=alpine_config_json,
        pe_class_binding=pe_class_binding,
